...
void kernel(unsigned int mtrows, unsigned int mtcols , float *mtdata, float *wdata,float *results){
/* exception if #Mrows!= #vectorpts */
	...
	for(tid = 0; tid < mtrows; tid++) {
		...
		for(x=0;x<mtcols;x++){
			...
			temp[x] = wdata[x]* mtdata[tid][x * rows + tid];
			sum += temp[x];
		}
...
int main(int argc ,char* argv[]) {
	...
	float* host_results = (float*) cudacudaMalloc(mtrows*sizeof(float));
	...
	unsigned char *dataT = (unsigned char*)cudacudaMalloc((size_t)size);
	unsigned char *dataT2 = (unsigned char*)cudacudaMalloc((size_t)size2);
